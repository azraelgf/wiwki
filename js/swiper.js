(() => {
    ...
    function initSliders() {
        if (document.querySelector(".slider-arrow__body")) new Swiper(".slider-arrow__body", {
        ...
        if (document.querySelector(".check-house__body")) new Swiper(".check-house__body", {
            ...
            navigation: {
                prevEl: ".check-house__btn_prev",
                nextEl: ".check-house__btn_next"
            },
            on: {
                init: function() {
                    updateNavTitles(this);
                },
        ...
        if (document.querySelector(".services-hero__body")) resizableSwiper("(max-width: 479.98px)", ".services-hero__body", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        if (document.querySelector(".exercises-items__body")) resizableSwiper("(max-width: 479.98px)", ".exercises-items__body", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        if (document.querySelector(".quests__body")) resizableSwiper("(max-width: 479.98px)", ".quests__body", {
            modules: [ Pagination ],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
        ...
        if (document.querySelector(".detail-gallery")) {
            new Swiper(".detail-gallery__main", {
            ...
            resizableSwiper("(min-width: 767.98px)", ".detail-gallery__thumbs", {
                modules: [ Thumb ],
                observer: true,
                direction: "vertical",
                observeParents: true,
        ...
        document.querySelectorAll(".slide-house__gallery").forEach(gallery => {
            document.querySelectorAll(".slide-house__gallery").forEach(gallery => {
                const mainEl = gallery.querySelector(":scope > .main-swiper");
                const thumbsEl = gallery.querySelector(":scope > .thumbs-swiper");
                if (!mainEl) return;
                const pagEl = mainEl.querySelector(".swiper-pagination");
                const main = mainEl.swiper || new Swiper(mainEl, {
                ...
                if (!thumbsEl) return;
                resizableSwiper("(min-width: 768px)", thumbsEl, {
                    modules: [ Thumb ],
                    slidesPerView: 5,
                    spaceBetween: 20,
                    freeMode: true,
                    ...
                    on: {
                        init() {
                            main.params.thumbs.swiper = this;
                            main.thumbs.init();
                            main.thumbs.update(true);
                        },
                        destroy() {
                            main.params.thumbs.swiper = null;
                            main.thumbs.init();
                        }
                    }
                });
        ...
        document.querySelectorAll(".slider-arrow").forEach(block => {
            const container = block.querySelector(".rest-detail__body.swiper");
            if (!container) return;
            const nextEl = block.querySelector(".swiper-controls .swiper-button-next");
            const prevEl = block.querySelector(".swiper-controls .swiper-button-prev");
    ...
    function updateNavTitles(swiper) {
        const prevBtn = document.querySelector(".check-house__btn_prev");
        const nextBtn = document.querySelector(".check-house__btn_next");
        const currentIndex = swiper.realIndex;
        const titles = [ ...document.querySelectorAll(".slide-house__title") ].map(el => el.textContent.trim());
        const prevIndex = (currentIndex - 1 + titles.length) % titles.length;
        const nextIndex = (currentIndex + 1) % titles.length;
        prevBtn.textContent = titles[prevIndex];
        nextBtn.textContent = titles[nextIndex];
    }
    const resizableSwiper = (breakpoint, selectorOrElements, swiperSettings, callback) => {
        const mql = window.matchMedia(breakpoint);
        const elements = Array.from(typeof selectorOrElements === "string" ? document.querySelectorAll(selectorOrElements) : selectorOrElements instanceof Element ? [ selectorOrElements ] : selectorOrElements);
        const enable = el => {
            if (el._swiperInstance) return;
            const instance = new Swiper(el, swiperSettings);
            el._swiperInstance = instance;
            if (callback) callback(instance, el);
        };
        const disable = el => {
            const inst = el._swiperInstance;
            if (inst && typeof inst.destroy === "function") inst.destroy(true, true);
            el._swiperInstance = null;
        };
        const checker = () => {
            if (mql.matches) elements.forEach(enable); else elements.forEach(disable);
        };
        ...
        return {
            refresh: checker,
            destroyAll() {
                mql.removeEventListener("change", checker);
